...
export class ProfesoresComponent implements OnInit { 
  ...
  private readonly profesorsService = inject(ProfesoresService); 
  private readonly router = inject(Router); 
 
  // Arreglo para almacenar el listado de profesors 
  ...
  getAllProfesors(){ 
    this.profesorsService.obtenerProfesores().subscribe({ 
...
// Metodo que permite navegar al formulario para insertar profesors 
navigateToForm(){ 
...
Swal.fire({ 
  ...
  if (result.isConfirmed){ 
    this.profesorsService.eliminarProfesor(event.target.value).subscribe({ 
      // En caso exitoso 
      next: (temp) => { 
        Swal.fire("Eliminado","Registro eliminado con exito","success"); 
        // Refrescamos la lista de profesors 
        this.getAllProfesors(); 
      }, 
      // En caso erroneo 
      error: (err) => { 
...
/** 
* Metodo que permite viajar al componente para agregar un profesor (pero en 
modo edicion). 
  */ 
updateProfesor(valor: number){ 
    // Viajando al componente agregar profesor 
